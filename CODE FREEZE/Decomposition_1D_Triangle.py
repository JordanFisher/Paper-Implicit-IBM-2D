...
for j in range(N):
    for k in range(N):
##        f[j + N * k] = abs(j-k)#cos(j)**2 + sin(k)**2 #+ cos(j**2) / (1 + abs(k)) + sin(3 * k) * cos(j**3)

##        f[j + N * k] = Phi256[abs(j - k)]
        f[j + N * k] = Phi64[abs(j - k)]        
##        f[j + N * k] = Phi64[min(N-1,abs(j - k))]
##        f[j + N * k] = Phi[(j - k) % N]
##        f[j + N * k] = 1. / (1 * ((j+5) * h - k * h)**2 + .001)
##        f[j + N * k] = 1. / (1 * (j * h + k * h)**2 + .001)
##        f[j + N * k] = 1. / (cos(4*pi*(j * h - k * h))**2 + 1)

##        f[j + N * k] = 1. / ((j * h - k * h)**2 + q)
...
def EvalDecompToPlane(z):
    ...
    return array(dot(matrix(z[:N]).T, matrix(z[N:])))

def EvalMultiDecompToPlane(Z):
    w = EvalDecompToPlane(Z[0])
    for l in range(1,M):
...
# Square predict setup
_Z = []
_ff = []
for l in range(M):
    _Z.append(zeros(N))
    _ff.append(zeros(N/2))
...
print "Square error: ", abs(_F - w).max()
for l in range(M):
    _Z[l][:N/2] = U[:,l]
    _Z[l][N/2:] = 0 * V[l,:]
...
# Solve for smaller triangle
for l in range(M):
    _ff[l] = dot(FF.T, _Z[l][:N/2])
...
for n in range(N/2):
    ...
    for l1 in range(M):
        for l2 in range(M):
            _M[l1, l2] = dot(_Z[l1][n:N/2], _Z[l2][n:N/2])
...
w = EvalMultiDecompToPlane(_Z)
w = ZeroTriangle(w)    
print "Sub triangle predict: ", abs(w - FF).max(), L2Norm(w - FF)
...
for i in range(30):
    ...
    for n in range(N):
        # Construct the MxM matrix
        _M = zeros((M,M))
        for l1 in range(M):
            for l2 in range(M):
                _M[l1, l2] = dot(Z[l1][N:N+n+1], Z[l2][N:N+n+1]) * h2
    ...
    # Renormalize
    for l in range(M):
        norm1 = dot(Z[l][:N], Z[l][:N])**.5
        norm2 = dot(Z[l][N:], Z[l][N:])**.5
        Z[l][:N] /= norm1
        Z[l][N:] /= norm2
        Z[l][:N] *= (norm1 * norm2)**.5
        Z[l][N:] *= (norm1 * norm2)**.5   

    w = EvalMultiDecompToPlane(Z)
    w = ZeroTriangle(w)
    print abs(w - F).max(), L2Norm(w - F)

    # Save best
    l2 = L2Norm(w - F)
    if (l2 < BestL2):
        BestL2 = l2
        BestW = EvalMultiDecompToPlane(Z)
    ...
    for n in range(N):
        ...
        for l1 in range(M):
            for l2 in range(M):
                _M[l1, l2] = dot(Z[l1][n:N], Z[l2][n:N])
...
def Interpolate(x):
    ...
    for j in range(N):
        for k in range(N):
            _x[2*j,2*k] = x[j,k]
            _x[2*j+1,2*k] = (x[j,k] + x[min(N-1,j+1),k]) / 2.
            _x[2*j,2*k+1] = (x[j,k] + x[j,min(N-1,k+1)]) / 2.
            _x[2*j+1,2*k+1] = (x[j,k] + x[min(N-1,j+1),min(N-1,k+1)]) / 2.
