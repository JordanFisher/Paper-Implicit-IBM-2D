...
Phi = array([ 0.25710939,  0.2472572 ,  0.22593886,  0.20450935,  0.18601602,
...
for j in range(N):
    for k in range(N):
##        f[j + N * k] = cos(j*h) * sin(k*h) + cos(j*h)**2 * sin(k*h)**2# + sin(cos(j*h)) + j * h * k * h + (k * h)**2 + (k*h)**.5 * (j*h)**.5 + (j * h + 1)**.3 * (k * h)**.6
