...
gridx = zeros((N,M),float64)
gridy = zeros((N,M),float64)
for j in range(N):
    gridx[j,:] = Domain_x * float(j) / N
for j in range(M):
    gridy[:,j] = Domain_y * float(j) / M
...
##        pf = 1. * f
##        pf = ModMultigridStep(__A,P,l,x-b,II,pf,1.)
##
...
##                e3[i] = z[i+Nb]
##                e3[i+Nb] = -z[i]
##        pf -= dot(pf,e1)*e1/dot(e1,e1) + dot(pf,e2)*e2/dot(e2,e2) + dot(pf,e3)*e3/dot(e3,e3)
##        pf -= dot(pf,e1)*e1/dot(e1,e1) + dot(pf,e2)*e2/dot(e2,e2) + dot(pf,e3)*e3/dot(e3,e3)
##        pf -= dot(pf,e1)*e1/dot(e1,e1) + dot(pf,e2)*e2/dot(e2,e2) + dot(pf,e3)*e3/dot(e3,e3)        
##        print dot(pf,e2), dot(pf,e1), dot(pf,e3)
##        xp = x + dt*dot(A,pf)
...
##        _e3 = ModMultigridStep(__A,P,l,e3,II,_e3,1.)    
    ...
    count = 0
    F = 0. * F
    while MaxRes > .5*dt and count < 4 and not ExplicitPredict or ExplicitPredict and count < 2:
        holdx = 1. * x
        count += 1
...
##        F = ModMultigridStep(__A,P,l,x-b,II,F,1.)
        ...
        subMax = 1000.
        SubCount = 0
        while SubCount < 3:#subMax > .000000000000001*_s:
            D = Jacobian(x, Nb, Links, Tethers).todense()
            eigs, vecs = Eig(dot(__A[0],D))
            eigs = abs(eigs)
            eigs.sort()
            print "Eigs eigs eigs!"
            print eigs
            print
            raw_input("")
...
##               pivot = 'middle', units = 'height', width = .003, scale = 30)
        ...
        Contour = contourf((vx - uy).T,[100,70,30,-30,-70,-100],origin='lower',extent=[0,2,0,1])
        colorbar()
        #clabel(Contour)
        scatter(X,Y,c='k',s=3)
