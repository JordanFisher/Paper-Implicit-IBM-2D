...
while CurT < T:
    ...
    count = 0
    F = 0. * F
    while MaxRes > .05*dt and count < 10 and not ExplicitPredict or ExplicitPredict and count < 2:
        holdx = 1. * x
        count += 1
...
##        f = F - _f
        ...
        holdx = 1. * x
        ArtificialStr = .7
        while SubCount < 5:#subMax > .000000000000001*_s:
            SubCount += 1
            ...
            D = D.todense() * _s - identity(2*Nb) / (ArtificialStr * __A[0][0,0])
            D = sparse.csc_matrix(D)
