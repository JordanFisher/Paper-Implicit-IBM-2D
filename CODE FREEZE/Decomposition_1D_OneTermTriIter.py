...
for j in range(N):
    for k in range(N):
##        f[j + N * k] = j - k# + sin(cos(j*h)) + j * h * k * h + (k * h)**2 + (k*h)**.5 * (j*h)**.5 + (j * h + 1)**.3 * (k * h)**.6
...
##        print abs(w - F).max()
    ...
    avg = (Z[:N] + Z[2*N:N-1:-1]) / 2.
    Z[:N] = Z[:N]
    Z[2*N:N-1:-1] = Z[2*N:N-1:-1]
...
for Iterations in range(1500):
    for l in range(M):        
        ...
        Z[l] = FindBestOneTerm(F2, holdz)


        w = EvalMultiDecompToPlane(Z)
        ZeroTriangle(w)
        print abs(w - F).max(), L2Norm(F-w)
        

        
    w = EvalMultiDecompToPlane(Z)
    ZeroTriangle(w)
    print "End of ", Iterations, "L2 Norm =", L2Norm(F-w)
##    raw_input("")
