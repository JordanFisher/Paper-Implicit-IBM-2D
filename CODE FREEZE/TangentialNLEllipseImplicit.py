...
l = .0 * EllipseCircumference (.4, .2) / Nb

for i in range(Nb-1):
#    l = .9*((X[i]-X[i+1])**2+(Y[i]-Y[i+1])**2)**.5
#    print ((X[i]-X[i+1])**2+(Y[i]-Y[i+1])**2)**.5, l
    Links.append( (i, i+1, _s, l) )
#l = .9*((X[Nb-1]-X[0])**2+(Y[Nb-1]-Y[0])**2)**.5
...
##    _b = I[0].T * b
##    NewtonRes = 100
##    while NewtonRes > .0000001:
##        FastForce(_x[:Nb2], _x[Nb2:], __f[:Nb2], __f[Nb2:], LinkIndices2, LinkData2, LinkCurL2, TetherIndex2, TetherData2)
##        _r = (_x-dot(__A[1], __f)) - _b
##        D = Jacobian(_x, Nb2, Links2, Tethers2).todense()
##
##        P = identity(2*Nb2) - __A[1] * D
##
##        _x -= linalg.solve(P,_r)
##
##        NewtonRes = max(abs(_r))
##        print "Coarse Residual:", NewtonRes
##
##    x = I[0] * _x
##
##    clf()
##    scatter(x[:Nb],x[Nb:])
##    scatter(_x[:Nb2],_x[Nb2:],c='r',s=20)
##    raw_input("")
...
##########################################################################################
    ...
    for i in range(Nb):
        for j in range(Nb):
            dist = min([abs(i-j), abs(i-Nb-j), abs(i-j+Nb)])
            if dist < width:
                decay = math.e**(-dist/.7)
                Band[i,j] = __A[0][i,j] * decay
                Band[i+Nb,j] = __A[0][i+Nb,j] * decay
                Band[i,j+Nb] = __A[0][i,j+Nb] * decay
                Band[i+Nb,j+Nb] = __A[0][i+Nb,j+Nb] * decay              
    ...
    BigChange = 100
    BigCount = 0
    while BigRes > .000001 and BigChange > .000001 and BigCount < 5:
        BigCount += 1
        
        # Construct projection matrices
        PN = zeros((2*Nb,2*Nb),float64)
        PT = zeros((2*Nb,2*Nb),float64)

        #_X, _Y = x[:Nb], x[Nb:]
        _X, _Y = Old_X, Old_Y
        for i in range(Nb):
            nx = _X[(i+1)%Nb] - 2*_X[i] + _X[(i-1)%Nb]
            ny = _Y[(i+1)%Nb] - 2*_Y[i] + _Y[(i-1)%Nb]
...
##        if BigCount == 1:
##            fn = dot(PN, _f)
##            b = holdb + dot(__A[0],fn)
        b = holdb - oldx

        NewtonRes = 100
        count = 0
        holdx = 1. * x
        while NewtonRes > .000005:
            count += 1
...
####            PT = identity(2*Nb) - PN
            ...
            FastForce(x[:Nb], x[Nb:], _f[:Nb], _f[Nb:], LinkIndices, LinkData, LinkCurL, TetherIndex, TetherData)
            #r = (x-dot(__A[0],_f)) - b
            r = (x-dot(__A[0],dot(PT,_f))) - b
            D = Jacobian(x, Nb, Links, Tethers).todense()

    #        P = identity(2*Nb) - __A[0] * D
    #        P = identity(2*Nb) - __A[0] * matrix(PT) * D
            #P = identity(2*Nb) - __A[0] * matrix(PT) * D * matrix(PT)
            P = PT - __A[0] * matrix(PT) * D * matrix(PT)

            #x -= 1 * dot(PT, linalg.solve(P,r))
            x -= 1 * linalg.solve(P,r)
            
##            print r
##            print linalg.solve(P,r)
##            print dot(PT, linalg.solve(P,r))
##            raw_input("")
            print "x:", z[0]

            NewtonRes = max(abs(r))
            print "Residual:", NewtonRes
            print


        BigChange = max(abs(x - holdx))
        x = .5 * holdx + .5 * x
        ...
        print "BigRes is ......", BigRes
        print "BigChange is ...", BigChange
        scatter(x[:Nb],x[Nb:])
        scatter(X,Y,c='r')
        raw_input("")
...
##########    count = 0
##########    F = 0. * F
##########    while MaxRes > .5*dt and count < 15 and not ExplicitPredict or ExplicitPredict and count < 2:
##########        holdx = 1. * x
##########        count += 1
##########
############        F = CG(__A[0],x-b,F)
...
##########        subMax = 1000.
##########        SubCount = 0
##########        while SubCount < 300:#subMax > .000000000000001*_s:
##########            SubCount += 1
##########            
##########            FastForce(x[:Nb], x[Nb:], _f[:Nb], _f[Nb:], LinkIndices, LinkData, LinkCurL, TetherIndex, TetherData)
##########            print _f
...
##########            # Factor out normal components
##########            for i in range(Nb):
##########                #nx, ny = -(x[(i+1)%Nb+Nb] - x[(i-1)%Nb+Nb]), x[(i+1)%Nb] - x[(i-1)%Nb]
##########                nx = x[(i-1)%Nb] - 2 * x[i] + x[(i+1)%Nb]
##########                ny = x[(i-1)%Nb+Nb] - 2 * x[i+Nb] + x[(i+1)%Nb+Nb]                
##########                n = (nx**2 + ny**2)**.5
##########                nx, ny = nx / n, ny / n
##########                nc = f[i] * nx + f[i+Nb] * ny
##########                f[i] -= nc * nx
...
##########            # Factor out normal components
##########            for i in range(Nb):
##########                #ny, nx = (x[(i+1)%Nb+Nb] - 2*x[i+Nb] + x[(i-1)%Nb+Nb]), x[(i+1)%Nb] - 2*x[i+Nb] + x[(i-1)%Nb]
##########                nx, ny = -(x[(i+1)%Nb+Nb] - x[(i-1)%Nb+Nb]), x[(i+1)%Nb] - x[(i-1)%Nb]
##########                n = (nx**2 + ny**2)**.5
##########                nx, ny = nx / n, ny / n
##########                nc = y[i] * nx + y[i+Nb] * ny
...
##########        clf()
##########        scatter(x[:Nb],x[Nb:])
##########        quiver(x[:Nb],x[Nb:],z[:Nb],z[Nb:])
##########        print "After explicit solve.."
##########        raw_input("")
##########
##########        
##########
