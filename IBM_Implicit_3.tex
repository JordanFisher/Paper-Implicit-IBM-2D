...
While more economical alternatives have been proposed recently for  some special cases, 
there is still a practical need for a computationally efficient approach that can be applied more broadly.
...
and obtain a rigorous estimate for this approximation. This matrix is expeditiously computed by using a combination of pre-computed
...
\begin{figure}[!b]
  ...
  \label{fig:loglog15}
\end{figure}
%Jordan: plot also line y=-2x +b for reader to see why order is close to 2
% Do not use log(e)  as y-label use instead log(error).
